...
from summit.komoot import bulk_prefix, download_segments, list_planned, parse_date, rename_tour
...
def test_parse_date(s, expected_none):
    result = parse_date(s)
    if expected_none:
...
class TestListPlanned:
    ...
    def test_returns_sorted_by_date_descending(self, capsys):
        tours = [
            {"id": 1, "name": "Old Ride", "date": "2024-01-01T00:00:00Z", "distance": 10000, "sport": "road"},
            {"id": 2, "name": "New Ride", "date": "2024-06-01T00:00:00Z", "distance": 20000, "sport": "road"},
        ]
    ...
    def test_prints_tour_info(self, capsys):
        tours = [
            {"id": 42, "name": "SEG-Test", "date": "2024-06-01T00:00:00Z", "distance": 5000, "sport": "road"},
        ]
        api = self._make_api(tours)
        list_planned(api)
    ...
    def test_handles_missing_date(self, capsys):
        tours = [
            {"id": 1, "name": "No Date Tour", "date": None, "distance": 1000, "sport": "mtb"},
        ]
        api = self._make_api(tours)
        result = list_planned(api)
        assert len(result) == 1

    def test_returns_list_of_dicts(self, capsys):
        tours = [
            {"id": 1, "name": "Ride", "date": "2024-06-01T00:00:00Z", "distance": 10000, "sport": "road"},
        ]
        api = self._make_api(tours)
        result = list_planned(api)
...
class TestBulkPrefix:
    ...
    def test_renames_matching_tours(self):
        ...
        assert ("1" not in [str(r[0]) for r in renamed] or any(r[2] == "NEW-Alpha" for r in renamed))
    ...
    def test_raises_on_patch_failure(self):
        ...
        api = self._make_api(tours)
        with patch("summit.komoot.requests.patch") as mock_patch:
            mock_patch.return_value = MagicMock(status_code=500, text="Server Error")
            with pytest.raises(RuntimeError, match="Rename failed"):
...
class TestDownloadSegments:
    ...
    def test_downloads_matching_tours(self, tmp_path):
        tours = [
    ...
    def test_skips_non_matching_tours(self, tmp_path):
    ...
    def test_mixed_success_failure(self, tmp_path, capsys):
        tours = [
            {"id": 1, "name": "SEG-Good"},
            {"id": 2, "name": "SEG-Bad"},
        ]
        ...
        def side_effect(tour_id, cache_dir):
            if tour_id == 2:
