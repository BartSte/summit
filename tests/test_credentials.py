...
class TestGetCredential:
    ...
    def test_direct_env_var(self, monkeypatch):
        monkeypatch.setenv("SUMMIT_GARMIN_PASSWORD", "secret123")
        assert get_credential("garmin", "password") == "secret123"

    def test_direct_env_var_takes_priority_over_cmd(self, monkeypatch):
    ...
    def test_direct_env_var_komoot_username(self, monkeypatch):
    ...
    def test_command_env_var_calls_subprocess(self, monkeypatch):
        monkeypatch.setenv("SUMMIT_GARMIN_PASSWORD_CMD", "rbw get 'Garmin Connect'")
    ...
    def test_command_stdout_stripped(self, monkeypatch):
        monkeypatch.setenv("SUMMIT_KOMOOT_USERNAME_CMD", "op item get Komoot --fields username")
    ...
    def test_command_non_zero_exit_raises_credential_error(self, monkeypatch):
    ...
    def test_command_non_zero_exit_includes_exit_code(self, monkeypatch):
        monkeypatch.setenv("SUMMIT_GARMIN_PASSWORD_CMD", "false")
        monkeypatch.delenv("SUMMIT_GARMIN_PASSWORD", raising=False)
    ...
    def test_missing_both_raises_credential_error(self, monkeypatch):
        monkeypatch.delenv("SUMMIT_GARMIN_PASSWORD", raising=False)
    ...
    def test_error_message_mentions_service_and_field(self, monkeypatch):
    ...
    def test_error_message_shows_direct_var_name(self, monkeypatch):
        monkeypatch.delenv("SUMMIT_GARMIN_PASSWORD", raising=False)
        monkeypatch.delenv("SUMMIT_GARMIN_PASSWORD_CMD", raising=False)
    ...
    def test_error_message_shows_cmd_var_name(self, monkeypatch):
        monkeypatch.delenv("SUMMIT_GARMIN_PASSWORD", raising=False)
        monkeypatch.delenv("SUMMIT_GARMIN_PASSWORD_CMD", raising=False)
